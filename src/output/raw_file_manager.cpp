...
void RawFileManager::initWriters(uint32_t min_round_sec)
{
	...
	ping_.writer = std::make_unique<DcsWriter>("ping", raw_dir_, collector_id_);
	stb_.writer = std::make_unique<DcsWriter>("cmcc_stb", raw_dir_, collector_id_);
	...
	ping_.writer->rotate(min_round_sec);
	stb_.writer->rotate(min_round_sec);
}
...
void RawFileManager::writeStbToSlot(WriterSlot &slot,
									const StbRecord &rec)
{
	std::string line = StbDcsSerializer::serialize(rec);
	if (line.empty())
		return;

	std::lock_guard<std::mutex> lk(slot.mu);
	slot.writer->writeLine(line.c_str(), line.size());
}
...
void RawFileManager::writePing(const PingRecord &rec)
{
	writeToSlot(ping_, [&](char *buf, size_t sz) -> size_t
				{ return PingDcsSerializer::serialize(rec, buf, sz); });
}

void RawFileManager::writeStb(const StbRecord &rec)
{
	writeStbToSlot(stb_, rec);
}
// ─────────────────────────────────────────────────────────
// 刷盘 / 关闭
// ─────────────────────────────────────────────────────────
