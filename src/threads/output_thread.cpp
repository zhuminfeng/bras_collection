...
void OutputThread::writePingRecord(const PingRecord &r)
{
	file_manager_->writePing(r);
	stat_ping_.fetch_add(1, std::memory_order_relaxed);
}

void OutputThread::writeStbRecord(const StbRecord &r)
{
	file_manager_->writeStb(r);
	stat_stb_.fetch_add(1, std::memory_order_relaxed);
}
...
uint32_t OutputThread::drainWorkerQueues(WorkerOutputQueues *wq)
{
	...
	{ // ★ Ping
		PingRecord r;
		uint32_t cnt = 0;
		while (cnt < batch && wq->ping_q.pop(r))
		{
			writePingRecord(r);
			++cnt;
		}
	...
	{ // ★ STB
		StbRecord r;
		uint32_t cnt = 0;
		while (cnt < batch && wq->stb_q.pop(r))
		{
			writeStbRecord(r);
			++cnt;
		}
...
OutputThread::QueueDepth OutputThread::getQueueDepth() const
{
	...
	for (uint16_t i = 0; i < nb_w; ++i)
	{
		...
		d.onu_total += worker_queues_[i]->onu_q.size();
		d.ping_total += worker_queues_[i]->ping_q.size();
		d.stb_total += worker_queues_[i]->stb_q.size();
	}
